...
/********************** External Functions *********************/
__host__ __device__ static __inline__ double cuCarg(dcomplex arg)
{
   return atan2(arg.y, arg.x);
}
/*===================================*/
__host__ __device__ static __inline__ dcomplex cuExp(dcomplex arg)
{
   dcomplex res;
   double s, c;
   double e = exp(arg.x);
...
/************************ CUDA Kernels ************************/
__global__ void Evolve_Potential(dcomplex *Y, double *V, double g, dcomplex tau)
{
	int k = blockIdx.x + blockIdx.y * gridDim.x;
	dcomplex q = make_cuDoubleComplex(0.0f,
			-tau.y * (V[k] + g*pow( cuCabs(Y[k]), 2 )) );
	Y[k] = cuCmul( Y[k], cuExp(q) );
}
/*===================================*/
__global__ void Evolve_Kinetic_Rotate(dcomplex *Y, double *ROT, dcomplex tau)
{
	int k = blockIdx.x + blockIdx.y * gridDim.x;
	dcomplex q = make_cuDoubleComplex( 0.0f,
			tau.y * ROT[blockIdx.x] );
	Y[k] = cuCmul(Y[k], cuExp(q) );
	Y[k] = cuCdiv( Y[k],  make_cuDoubleComplex( (double)NX, 0.0f ) );
}
...
public:
		...
		dt(dt_), tmax(tmax_), logf(logf_)
	{
		...
		double C[3] = {
				...
				2.0f/3.0f };
		for(int k=0; k<3; k++)
			SIC[k] = make_cuDoubleComplex(0.0f, dt*C[k]);
		SIC[3] = make_cuDoubleComplex(0.0f, 2.0f*SIC[0].y);
		SIC[4] = make_cuDoubleComplex(0.0f, -1.0f*SIC[0].y);
	}
	/*=======================================================*/
	void update()
	{
